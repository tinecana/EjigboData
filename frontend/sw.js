...
const CACHE_NAME = "pwms-shell-v3";
...
self.addEventListener("fetch", (event) => {
  ...
  // The HTML document itself: NETWORK-FIRST. This is the one resource
  // that changes constantly during development/deployment — serving it
  // cache-first would mean the app can appear to "hang" on an old,
  // possibly broken snapshot instead of picking up the current file.
  // The cache is only used as a fallback when the network is truly
  // unavailable (the actual offline-first use case).
  const isDocumentRequest = req.mode === "navigate" || url.pathname.endsWith(".html") || url.pathname === "/" || url.pathname.endsWith("/");
  ...
  // Everything else same-origin (future static assets: icons, fonts, etc.):
  // cache-first for instant offline load, with a background refresh.
  event.respondWith(
